...
import { useAdminUpdateBlogMutation, useAdminGetAllBlogsQuery } from '@/redux/features/admin/adminApi';
...
const EditBlogPage = () => {
  ...
  const { data: blogsData } = useAdminGetAllBlogsQuery();
  const [updateBlog, { isLoading }] = useAdminUpdateBlogMutation();
  ...
  useEffect(() => {
    if (blogsData?.data && id) {
      const blog = blogsData.data.find((b) => b._id === id);
      if (blog) {
        setFormData({
          title: blog.title || '',
          slug: blog.slug || '',
          ...
          img: blog.img || '',
          list_img: blog.list_img || '',
          author: blog.author || '',
          category: blog.category || 'electronics',
  ...
  return (
    <AdminLayout>
      <div>
        <h1 style={{ marginBottom: '30px', color: '#2c3e50' }}>Edit Blog</h1>
