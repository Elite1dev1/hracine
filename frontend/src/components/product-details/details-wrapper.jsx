  ...
  return (
    <div className="tp-product-details-wrapper">
      ...
      {keyBenefits && keyBenefits.length > 0 && (
        <div className="tp-product-details-benefits mb-20" style={{ marginTop: '20px' }}>
          <h4 style={{ fontSize: '18px', fontWeight: '600', marginBottom: '12px', color: '#22160E' }}>Key Benefits</h4>
          <ul style={{ paddingLeft: '20px', margin: 0, listStyle: 'none' }}>
            {keyBenefits.map((benefit, index) => (
              <li key={index} style={{ marginBottom: '8px', fontSize: '14px', color: '#55585B', lineHeight: '1.6', position: 'relative', paddingLeft: '20px' }}>
                <span style={{ position: 'absolute', left: 0, color: '#C47070' }}>•</span>
                {benefit}
              </li>
            ))}
          </ul>
      ...
      {ingredients && (
        <div className="tp-product-details-ingredients mb-20" style={{ marginTop: '20px', padding: '15px', backgroundColor: '#F8F8F8', borderRadius: '5px' }}>
          <h4 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '10px', color: '#22160E' }}>Ingredients</h4>
          <p style={{ fontSize: '14px', color: '#55585B', lineHeight: '1.6', margin: 0 }}>
            {ingredients}
          </p>
        </div>
      )}
      ...
      {howToUse && (
        <div className="tp-product-details-howtouse mb-20" style={{ marginTop: '20px', padding: '15px', backgroundColor: '#F8F8F8', borderRadius: '5px' }}>
          <h4 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '10px', color: '#22160E' }}>How to Use</h4>
          <p style={{ fontSize: '14px', color: '#55585B', lineHeight: '1.6', margin: 0, whiteSpace: 'pre-line' }}>
            {howToUse}
          </p>
        </div>
      )}
        ...
        {discount > 0 ? (
          <>
            ...
            <span className="tp-product-details-price new-price">
              {" "}{formatCurrency(Number(price) - (Number(price) * Number(discount)) / 100)}
            </span>
          </>
        ) : (
