...
const BlogItem = ({ item = {} }) => {
  ...
  const blogIdentifier = item.slug || item.id || item._id;
  return (
    <>
      <article
          ...
          !item.slider && (
            <div className="tp-postbox-thumb w-img">
              <Link href={`/blog-details/${blogIdentifier}`}>
                <Image src={item.img || ''} alt={item.title || "blog img"} width={800} height={400} style={{width:'100%',height:'auto'}} />
              </Link>
            </div>
          )}
        {item.video && (
        ...
        {!item.blockquote && (
          <div className="tp-postbox-content">
            ...
            <div className="tp-postbox-text">
              <p>
                {item.desc || item.sm_desc || 'No description available'}
                {(item.desc || item.sm_desc) ? ' […]' : ''}
              </p>
            </div>
            <div className="tp-postbox-read-more">
              <Link href={`/blog-details/${blogIdentifier}`} className="tp-btn">
                Read More
              </Link>
            </div>
          </div>
