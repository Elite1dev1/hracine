        ...
        <table className="table">
          ...
          <tbody>
            {order_items.map((item, i) => {
              ...
              return (
                <tr key={i}>
                  <th scope="row">#{item.invoice || item._id.substring(20, 25)}</th>
                  <td data-info="title">
                    {dayjs(item.createdAt).format("MMMM D, YYYY")}
                  </td>
                  <td
                    data-info={`status ${status === "pending" ? "pending" : ""}  ${status === "processing" ? "hold" : ""}  ${status === "delivered" ? "done" : ""}`}
                    className={`status ${status === "pending" ? "pending" : ""} ${status === "processing" ? "hold" : ""}  ${status === "delivered" ? "done" : ""}`}
                  >
